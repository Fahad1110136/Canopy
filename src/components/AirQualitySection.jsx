...
import { Search, RefreshCw, AlertTriangle, Wind, SearchX } from 'lucide-react'
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {globalError && (
          <div className="mb-8 flex items-start gap-3 rounded-2xl border border-(--color-line) bg-(--color-gold-soft)/40 p-5">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-(--color-forest-deep)">{describeError(globalError)}</p>
              <p className="mt-1 text-sm text-(--color-ink-soft)">
                This can happen if the network request failed or the service is temporarily unavailable.
              </p>
            </div>
            <button onClick={() => setReloadToken((t) => t + 1)} className="shrink-0 inline-flex items-center gap-1.5 text-sm font-medium text-(--color-forest-deep) hover:text-(--color-forest) visible-focus">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading &&
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="animate-pulse rounded-2xl border border-(--color-line) bg-(--color-surface) p-5 h-[104px]" />
            ))}
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-sm font-medium text-(--color-ink)">
                      <span aria-hidden="true">{loc.flag}</span>{loc.name}
                    </span>
                  ...
                  {ok ? (
                    <>
                      <p className={`mt-3 font-display text-2xl ${colors.text}`}>
                        {Math.round(loc.data.aqi)}<span className="text-sm font-body text-(--color-ink-soft) ml-1">EU AQI</span>
                      </p>
                      <div className="mt-2.5 h-1.5 rounded-full bg-(--color-bg) overflow-hidden">
                        <div className={`h-full rounded-full ${colors.bar}`} style={{ width: `${Math.min(100, loc.data.aqi)}%` }} />
                      </div>
                      <p className="mt-2 text-[11px] text-(--color-ink-soft)">PM2.5 {loc.data.pm2_5} µg/m³ · O₃ {loc.data.ozone} µg/m³</p>
                    </>
                  ) : (
                    <p className="mt-3 flex items-center gap-1.5 text-sm text-(--color-ink-soft)">
                      <Wind size={13} className="shrink-0" /> {describeError(loc.error)}
                    </p>
                  )}
                </motion.div>
              )
        ...
        {!loading && filtered.length === 0 && (
          <EmptyState
            ...
            description={`Nothing found for "${query}". Try a different city or country name.`}
            className="mt-4"
          />
        )}
      </div>
