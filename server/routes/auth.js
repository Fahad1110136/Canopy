...
router.post('/signup', async (req, res) => {
  ...
  if (companyMode === 'create') {
    ...
    company = await getCompanyByJoinCode(joinCode)
    if (!company) {
...
router.get('/verify', async (req, res) => {
  ...
  res.json({ token: jwt, user: await withCompany(freshUser) })
})
...
router.post('/resend-verification', async (req, res) => {
  ...
  const verificationToken = generateVerificationToken()
  const verificationTokenExpiry = new Date(Date.now() + VERIFICATION_TOKEN_TTL_MS).toISOString()
  await setVerificationToken(user.id, verificationToken, verificationTokenExpiry)
...
router.post('/login', async (req, res) => {
  ...
  res.json({ token, user: await withCompany(user) })
})
...
router.get('/me', requireAuth, async (req, res) => {
  ...
  res.json(await withCompany(user))
})
